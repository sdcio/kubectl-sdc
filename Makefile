.PHONY: build
build:
	CGO_ENABLED=0 go build ./cmd/kubectl-sdc
...
mocks-gen: mocks-rm ## Generate mocks for all the defined interfaces.
	mkdir -p $(MOCKDIR)
	mkdir -p $(MOCKDIR)/blame $(MOCKDIR)/apply $(MOCKDIR)/deviations $(MOCKDIR)/runningconfig
	...
	mockgen -package=mockrunningconfig -source=pkg/commands/runningconfig/runningconfig.go -destination=$(MOCKDIR)/runningconfig/runningconfig.go
